...
|---|---|---|---|
| 1 | `eco_fenets_derive_queries.py` | Walk rtl_diff and emit complete query list, incl. Cat-4d comb_net_force selector conditions (deterministic — replaces hand-picked agent reasoning) | `<AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_queries_raw.json` |
| 2 | `eco_fenets_sanitize_queries.py` | Collapse duplicate `<scope>/<scope>/` segments (rule-based clean-up) | `<AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_queries.json` |
| 3 | *(agent submits FM via TileBuilder)* | Run find_equivalent_nets per target, handle FM-036 retries, copy raw rpts | `<AI_ECO_FLOW_DIR>/<TAG>_find_equivalent_nets_raw*.rpt` |
| 4 | `eco_fenets_rename_map.py` | Parse all raw rpts → emit per-stage rename map JSON (Step 3 reads this FIRST) | `<AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_rename_map.json` |
| 5 | `eco_fenets_chain.py` **(ONLY if rtl_diff has `comb_net_force`)** | Per-stage chaining (STEP D-CHAIN): resolve PP then Route selector conditions off the Synth net (survival shortcut + FM). Without it PP/Route stay FM-036 and C10 fails. | updates `<AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_rename_map.json` |
| 6 | `eco_validate_step2.py` | Final step2→step3 gate; **C10** hard-fails if any comb_net_force selector condition is unresolved in any stage | `<AI_ECO_FLOW_DIR>/<TAG>_eco_validate_step2.json` |
...
python3 script/eco_scripts/eco_fenets_sanitize_queries.py \
    --queries-in  <AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_queries_raw.json \
    --queries-out <AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_queries.json
```
...
- If retries exhaust, write the failing entries to `<AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_unresolved.json` for escalation.
...
- **DO NOT add to `queries.json`.** Append to `<AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_agent_added.json` with explicit `category: 99` + `source: "agent_added: <reason>"`. Submit those separately.
...
- Do NOT poll `<AI_ECO_FLOW_DIR>/<fenets_tag>_spec` — rpt files are authoritative

**B3. Read:** `cat <AI_ECO_FLOW_DIR>/<fenets_tag>_spec`
...
cp <AI_ECO_FLOW_DIR>/<fenets_tag>_find_equivalent_nets_raw.rpt <AI_ECO_FLOW_DIR>/
...
python3 script/eco_scripts/eco_fenets_rename_map.py \
    ...
    --raw-files <AI_ECO_FLOW_DIR>/<fenets_tag>_find_equivalent_nets_raw.rpt \
                <AI_ECO_FLOW_DIR>/<retry_tag>_find_equivalent_nets_raw_*.rpt \
    --tag       <TAG> \
    --tile      <TILE> \
    --ref-dir   <REF_DIR> \
...
**CRITICAL — pass `--raw-files`, NOT `--raw-dir`.** Using `--raw-dir` on a shared directory picks up every raw rpt from every previous run in the directory, polluting the rename map with stale data from other ECOs. Pass ONLY the raw rpt files generated by THIS run's FM queries (the fenets_tags you submitted in STEP B). Include all retry tags: `<AI_ECO_FLOW_DIR>/<fenets_tag>_find_equivalent_nets_raw.rpt`, `<AI_ECO_FLOW_DIR>/<retry_tag>_find_equivalent_nets_raw_fm036_retry*.rpt`, etc.
...
**Skip this step entirely ONLY if the rtl_diff has NONE of `comb_net_force`, a register-guard `and_term`, NOR a `wire_swap` with `condition_inputs_to_query`.** (Grep: `grep -qE 'comb_net_force|"target_register"|condition_inputs_to_query' <AI_ECO_FLOW_DIR>/<TAG>_eco_rtl_diff.json`.)
...
python3 script/eco_scripts/eco_fenets_chain.py \
    ...
    --ref-dir    <REF_DIR> \
    --output     <AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_rename_map.json \
    > <AI_ECO_FLOW_DIR>/<TAG>_chain_PrePlace_queries.txt
```
...
```bash
ECO_OUT_DIR=<AI_ECO_FLOW_DIR> python3 script/genie_cli.py \
  -i "find equivalent nets at <REF_DIR> for <TILE> netName:<comma-joined lines from <AI_ECO_FLOW_DIR>/<TAG>_chain_PrePlace_queries.txt>" \
  --execute --xterm
```
...
} > <AI_ECO_FLOW_DIR>/<chain_tag>_find_equivalent_nets_raw_chain_PrePlace.rpt
cp <AI_ECO_FLOW_DIR>/<chain_tag>_find_equivalent_nets_raw_chain_PrePlace.rpt <AI_ECO_FLOW_DIR>/
```
...
python3 script/eco_scripts/eco_fenets_chain.py \
    ...
    --rtl-diff   <AI_ECO_FLOW_DIR>/<TAG>_eco_rtl_diff.json \
    --ref-dir    <REF_DIR> \
    --raw-rpt    <AI_ECO_FLOW_DIR>/<chain_tag>_find_equivalent_nets_raw_chain_PrePlace.rpt \
    --output     <AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_rename_map.json
```
...
python3 script/eco_scripts/eco_fenets_chain.py --mode emit-nets --stage Route \
    --rename-map <AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_rename_map.json --rtl-diff <AI_ECO_FLOW_DIR>/<TAG>_eco_rtl_diff.json \
    --ref-dir <REF_DIR> --output <AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_rename_map.json \
    > <AI_ECO_FLOW_DIR>/<TAG>_chain_Route_queries.txt
...
python3 script/eco_scripts/eco_fenets_chain.py --mode merge --stage Route \
    --rename-map <AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_rename_map.json --rtl-diff <AI_ECO_FLOW_DIR>/<TAG>_eco_rtl_diff.json \
    --ref-dir <REF_DIR> --raw-rpt <AI_ECO_FLOW_DIR>/<chain_tag2>_find_equivalent_nets_raw_chain_Route.rpt \
    --output <AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_rename_map.json
```
...
Write `<AI_ECO_FLOW_DIR>/<TAG>_eco_step2_fenets.rpt` using this exact format:
...
```
SPEC_SOURCES:
  Synthesize: <AI_ECO_FLOW_DIR>/<fenets_tag>_spec
  PrePlace:   FALLBACK
  Route:      <AI_ECO_FLOW_DIR>/<fm036_retry1_tag>_spec
```
...
Every raw rpt file written MUST exist flat directly in `AI_ECO_FLOW_DIR/`. Verify each individually:
...
```bash
cp <AI_ECO_FLOW_DIR>/<tag>_find_equivalent_nets_raw*.rpt <AI_ECO_FLOW_DIR>/
```
...
python3 script/eco_scripts/eco_validate_step2.py \
    --queries     <AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_queries.json \
    --queries-raw <AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_queries_raw.json \
    --raw-rpts    <AI_ECO_FLOW_DIR>/<FENETS_TAG>_find_equivalent_nets_raw*.rpt \
    --rename-map  <AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_rename_map.json \
    --rtl-diff    <AI_ECO_FLOW_DIR>/<TAG>_eco_rtl_diff.json \
    --ref-dir     <REF_DIR> \
    --output      <AI_ECO_FLOW_DIR>/<TAG>_eco_validate_step2.json
```
...
Read `<AI_ECO_FLOW_DIR>/<TAG>_eco_validate_step2.json` issues list. For each issue, build a targeted re-query:
...
Write `<AI_ECO_FLOW_DIR>/<TAG>_eco_step2_fenets_rerun_round<ROUND>.rpt`:
...
Write `<AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_rerun_round<ROUND>.json`:
```json
{
  "round": <ROUND>,
