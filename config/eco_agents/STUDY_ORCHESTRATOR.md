...
```bash
ls <AI_ECO_FLOW_DIR>/data/<TAG>_round_handoff.json 2>/dev/null && echo EXISTS
```
...
```bash
ls <AI_ECO_FLOW_DIR>/data/<TAG>_pending_spawn.txt 2>/dev/null && cat <AI_ECO_FLOW_DIR>/data/<TAG>_pending_spawn.txt
```
...
5. Write `<AI_ECO_FLOW_DIR>/data/<TAG>_eco_analyze` metadata file:
   ```
   tile=<TILE>
   ref_dir=<REF_DIR>
   tag=<TAG>
...
Wait for the sub-agent to complete and read `<AI_ECO_FLOW_DIR>/data/<TAG>_eco_rtl_diff.json`.

**CHECKPOINT:** Verify `<AI_ECO_FLOW_DIR>/data/<TAG>_eco_rtl_diff.json` exists and contains at least one entry in `changes[]` and `nets_to_query[]` before proceeding. If missing or empty — the sub-agent failed. Do NOT continue to Step 2.
...
cd <BASE_DIR> && python3 script/eco_scripts/eco_validate_step1.py \
    ...
    --ref-dir  <REF_DIR> \
    --output   <AI_ECO_FLOW_DIR>/data/<TAG>_eco_validate_step1.json
STEP1_EXIT=$?
```
...
```python
import json, glob, os
canon = f"{AI_ECO_FLOW_DIR}/data/{TAG}_eco_validate_step1.json"
if not os.path.exists(canon):                      # written ONLY on pass
    dbg = glob.glob(f"{AI_ECO_FLOW_DIR}/data/{TAG}_eco_validate_step1_iter*.json")
...
```
**Step 2 MUST NOT run unless the canonical `<AI_ECO_FLOW_DIR>/data/<TAG>_eco_validate_step1.json` exists (== overall_pass). No exceptions.**
...
python3 script/eco_scripts/eco_fenets_derive_queries.py \
    ...
    --tile     <TILE> \
    --ref-dir  <REF_DIR> \
    --output   <AI_ECO_FLOW_DIR>/data/<TAG>_eco_fenets_queries_raw.json
```
...
- Path to RTL diff JSON: `<AI_ECO_FLOW_DIR>/data/<TAG>_eco_rtl_diff.json`
- Pre-derived raw query list: `<AI_ECO_FLOW_DIR>/data/<TAG>_eco_fenets_queries_raw.json`
...
cp <AI_ECO_FLOW_DIR>/data/<TAG>_eco_fenets_rename_map.json <AI_ECO_FLOW_DIR>/
```
...
canon = '<AI_ECO_FLOW_DIR>/data/<TAG>_eco_validate_step2.json'
if os.path.exists(canon):
    print('Step 2 validator PASSED — proceeding to Step 3'); sys.exit(0)
dbg = glob.glob('<AI_ECO_FLOW_DIR>/data/<TAG>_eco_validate_step2_iter*.json')
if not dbg:
...
ls <AI_ECO_FLOW_DIR>/data/<TAG>_eco_fenets_queries_sanitize_marker.txt || \
...
**Extract SPEC_SOURCES from `<AI_ECO_FLOW_DIR>/data/<TAG>_eco_step2_fenets.rpt`** (read the SPEC_SOURCES section at the bottom of the RPT) — pass these to the Step 3 sub-agent prompt.
...
```
Read the output JSON and **pass it explicitly to the eco_netlist_studier sub-agent prompt** as `GAP15_CHECK_PATH=<AI_ECO_FLOW_DIR>/data/<TAG>_eco_and_term_port_check.json`. The studier reads this file to get `is_output_port` and `strategy` for each `and_term` change — it does NOT re-derive these itself.
...
- The RTL diff JSON at `<AI_ECO_FLOW_DIR>/data/<TAG>_eco_rtl_diff.json` (provides old_net/new_net per change)
- **ALL spec file paths** from Step 2 — initial run AND every retry:
  - Initial: `<AI_ECO_FLOW_DIR>/data/<fenets_tag>_spec`
  - No-Equiv-Nets retries: `<AI_ECO_FLOW_DIR>/data/<noequiv_retry1_tag>_spec`, `<AI_ECO_FLOW_DIR>/data/<noequiv_retry2_tag>_spec` (if they exist)
  - FM-036 retries: `<AI_ECO_FLOW_DIR>/data/<fm036_retry1_tag>_spec`, `<AI_ECO_FLOW_DIR>/data/<fm036_retry2_tag>_spec`, `<AI_ECO_FLOW_DIR>/data/<fm036_retry3_tag>_spec` (if they exist)
  ...
  for retry_tag, retry_spec_path in noequiv_retries:   # in order retry1, retry2
      ...
      for stage in ["Synthesize", "PrePlace", "Route"]:
          if stage_has_qualifying_cells(retry_raw, stage):
              spec_sources[stage] = f"{AI_ECO_FLOW_DIR}/data/{retry_tag}_spec"
...
- Output: `<AI_ECO_FLOW_DIR>/data/<TAG>_eco_preeco_study.json` (schema defined in `eco_netlist_studier.md`)

**CHECKPOINT 3a (MANDATORY — verify studier output before running the emitters):**
```bash
ls -la <AI_ECO_FLOW_DIR>/data/<TAG>_eco_preeco_study.json
python3 -c "import json; d=json.load(open('<AI_ECO_FLOW_DIR>/data/<TAG>_eco_preeco_study.json')); assert any(d.get(s) for s in ['Synthesize','PrePlace','Route']), 'all stages empty'"
ls <AI_ECO_FLOW_DIR>/data/<TAG>_eco_step3_collect.rpt
```
...
**MANDATORY post-Step 3: Run eco_validate_step3.py in a CATCH-AND-FIX LOOP** (validator is expensive — run it ONLY inside the loop, not separately). Each pass ALWAYS writes a per-iteration debug file `<AI_ECO_FLOW_DIR>/data/<TAG>_eco_validate_step3_iter$i.json`; the canonical `<AI_ECO_FLOW_DIR>/data/<TAG>_eco_validate_step3.json` is written ONLY when a pass succeeds and is REMOVED on failure — so it exists **iff the latest run passed** and always contains a PASSING result. The fixer therefore reads the FAILING issues from the per-iteration file, not the canonical:
...
for i in 1 2 3; do
  python3 script/eco_scripts/eco_validate_step3.py \
      ...
      --output <AI_ECO_FLOW_DIR>/data/<TAG>_eco_validate_step3.json --iter $i
  ...
  python3 script/eco_scripts/eco_study_fixer.py \
      --study   <AI_ECO_FLOW_DIR>/data/<TAG>_eco_preeco_study.json \
      --issues  <AI_ECO_FLOW_DIR>/data/<TAG>_eco_validate_step3_iter$i.json \
      --rtl-diff <AI_ECO_FLOW_DIR>/data/<TAG>_eco_rtl_diff.json \
      --ref-dir <REF_DIR> \
      --raw-rpts data/*_find_equivalent_nets_raw*.rpt \
      --step2-rpt <AI_ECO_FLOW_DIR>/data/<TAG>_eco_step2_fenets.rpt \
      --output  <AI_ECO_FLOW_DIR>/data/<TAG>_eco_preeco_study.json
done
# After the loop, <AI_ECO_FLOW_DIR>/data/<TAG>_eco_validate_step3.json exists ONLY if Step 3 passed.
...
3. **Re-run `eco_validate_step3.py` exactly ONCE** after the batch (it auto-writes a fresh `_iter<N>.json`; PASS is signalled by the canonical `<AI_ECO_FLOW_DIR>/data/<TAG>_eco_validate_step3.json` appearing — its presence == `passed: true`). Read the next round's issues from the new newest `_iter<N>.json`.
...
python3 script/eco_scripts/eco_functional_precheck.py \
    ...
    --ref-dir  <REF_DIR> \
    --jira     <JIRA> \
    --output   <AI_ECO_FLOW_DIR>/data/<TAG>_eco_functional_precheck.json
cp <AI_ECO_FLOW_DIR>/data/<TAG>_eco_functional_precheck.json <AI_ECO_FLOW_DIR>/ 2>/dev/null || true
```

Verify stdout contains `ECO_SCRIPT_LAUNCHED: eco_functional_precheck.py`. It always writes
`<AI_ECO_FLOW_DIR>/data/<TAG>_eco_functional_precheck.json` with a `passed` bool (`passed=false` on any FAIL).
...
cd <BASE_DIR> && python3 script/eco_scripts/eco_rpt_generator.py step3 \
    ...
    --output <AI_ECO_FLOW_DIR>/data/<TAG>_eco_step3_netlist_study_round1.rpt
```
...
```bash
cp <AI_ECO_FLOW_DIR>/data/<TAG>_eco_step3_netlist_study_round1.rpt <AI_ECO_FLOW_DIR>/
...
```bash
cat > <AI_ECO_FLOW_DIR>/data/<TAG>_phase_a_handoff.json <<JSON_EOF
{
  "tag":             "<TAG>",
  "ref_dir":         "<REF_DIR>",
  "tile":            "<TILE>",
  ...
  "artifacts": {
    ...
    "fenets_rename_map": "<AI_ECO_FLOW_DIR>/data/<TAG>_eco_fenets_rename_map.json",
    "preeco_study":      "<AI_ECO_FLOW_DIR>/data/<TAG>_eco_preeco_study.json"
  }
}
JSON_EOF
```
...
```bash
ls -la <AI_ECO_FLOW_DIR>/data/<TAG>_phase_a_handoff.json
python3 -c "import json; json.loads(open('<AI_ECO_FLOW_DIR>/data/<TAG>_phase_a_handoff.json').read())" && echo "handoff valid"
```
...
```bash
date -Iseconds | xargs -I{} echo "exited {}" > <AI_ECO_FLOW_DIR>/data/<TAG>_study_phase_exited.marker
ls -la <AI_ECO_FLOW_DIR>/data/<TAG>_study_phase_exited.marker
```
...
STUDY phase complete (Steps 1-3).
  phase_a_handoff: <AI_ECO_FLOW_DIR>/data/<TAG>_phase_a_handoff.json
  exit_sentinel:   <AI_ECO_FLOW_DIR>/data/<TAG>_study_phase_exited.marker
