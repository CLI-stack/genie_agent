...
> **Register guard-change (`target_register` + `branch_assigns` OR `branch_loads`) is NOT hand-built here.** If the change carries `target_register` AND either `branch_assigns` (constant-assign guard broaden/tighten) OR `branch_loads` (data-load on a clock-gated register/counter), do NOT hand-build the combine, the `.D` rewire, or any `E`-pin/enable rewire — the deterministic `eco_cone_rebuild.py --emit-into-study` pass (`emit_reg_guard_delta_batch`) owns BOTH classes, correct-by-construction: constant → OR vs AND-NOT chosen by the assigned value (fixing the JIRA-9666 `postcas` clear-branch bug); data-load/clock-gated → slim per-bit `.D` hold-mux (`load_active ? load_val : orig-.D`) + clock-gate enable widen (`E = old_E | load_active`) (fixing the JIRA-9666 `WckSyncCtr0` OR-into-E-only bug). Do not "fix" either here with a hand OR2. The GAP-24 recipe below is for combinational term-folds only (no `branch_assigns` and no `branch_loads`).
