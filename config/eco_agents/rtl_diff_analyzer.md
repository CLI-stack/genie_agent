...
**`priority_force` — a new branch that FORCES signals to constants (do NOT force-fit into and_term/enable_swap).** When the ECO adds a branch like `else if (<cond>) begin sigA <= CONST_A; sigB <= CONST_B; end` (e.g. LPDDR5 WCK-not-in-sync → `recdsp_c0vld <= 1'b1; recdsp_c0mop <= UMC_MOP_CAS;`), classify as `change_type: priority_force` with: `condition_expr`, `condition_gate_chain` (build `<cond>` as real gates — NEVER leave it as `PENDING_FM_RESOLUTION`/a synthetic net to query), and `forced_signals: [{signal, const, old_next_state_net}]` (one per driven signal — do NOT drop `recdsp_c0vld`). Step 3 builds, per forced signal, a force-mux `AO22(cond, CONST, ~cond, old_next_state) → n_eco_<jira>_<seq>` and a DFF-pin rewire of that signal's `.D` to the fresh net (never a driver-rename, never a constant left dangling). Validator: Step 1 requires `condition_gate_chain` + each `forced_signals[].const` a valid constant + `old_next_state_net`; Step 3 requires a force-mux (fresh output) + `.D` rewire for EVERY forced signal.
