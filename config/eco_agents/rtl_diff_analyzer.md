...
> **DATA-LOAD / clock-gated register (counter) — set `branch_loads` instead of `branch_assigns`.** When the widened branch LOADS a DATA expression rather than a constant (e.g. a clock-gated counter `WckSyncCtr0 <= rdwcksyncclks`), set `target_register` + **`branch_loads`** = the loaded expression (e.g. `"rdwcksyncclks"`). The SAME deterministic builder (`eco_cone_rebuild.py emit_reg_guard_delta_batch`) handles it with input-pin rewires on every cell that consumes the old guard signal — no output-pin renames, no mux gates inserted (see `old_guard_net` below). Do NOT model it as a bare `enable_swap` / single-net OR into the `E`-pin — that enables the update on the new region but leaves the `.D` data-select unchanged, so the counter loads the WRONG value (JIRA-9666 `WckSyncCtr0`: loaded the decrement path instead of `rdwcksyncclks`). Note: any guard signal synthesis folded out of the netlist (e.g. `recdsp_c0cs`, an internal combinational reg) is BOUND to its existing FM-equivalent net via **Step-2 fenets (Cat 4e — reg_guard_folded_conditions, chained to PP/Route)** rather than rebuilt; the Step-1 (`branch_loads` required) and Step-3 (clock-gate effective-flop oracle) guards hard-fail so a wrong counter can never ship.
>
> **`old_guard_net` (MANDATORY for clock-gated, RECOMMENDED for simple-flop).** The synthesized netlist signal that encodes the OLD branch guard (e.g. `ctmn_1251735` = synthesis form of `mop==RD`). The builder uses this to find every existing cell in the module whose input pin carries `old_guard_net` and rewires that pin to the new guard expression — the **input-pin-rewire** methodology (confirmed from JIRA-9666 Conformal ECO analysis). This is critical for PPvsSynth FM: the driver of the E/D output net may differ between Synthesize and PP/Route (P&R renames it), but the cell *consuming* the old guard net is the same instance in all stages (e.g. `ctmi_1636536` is identical in Synth and PP). No output pin is renamed; no mux gate is inserted. **How to find it:** grep the PreEco Synthesize netlist for the cell driving the clock-gate E-net (from `_reg_clockgate()` output), then look at which of its input pins carries the old guard condition. For 9666: `ctmi_1636536` drives `N294` (E-pin); its input `.A2 = ctmn_1251735` → `old_guard_net = "ctmn_1251735"`. The builder resolves per-stage consumers automatically via `_consumer_map()`.
>
> **`suppress_driver_side` (optional bool).** Set to `true` when the guard change for a simple (non-clock-gated) flop is handled by a separate `comb_net_force` / Intent-B change in the same ECO and the builder should NOT emit a driver-side output rename for this register. Example: JIRA-9666 `postcas` — its guard cone is corrected by the `A584340` input-pin rewire in Intent-B; emitting a driver-side rename of `ctmi_1636715.ZN` in Intent-A would create structural divergence between Synth and PP/Route (engineer does NOT touch `ctmi_1636715`). When `suppress_driver_side: true`, the builder skips this register entirely — no rewire emitted from Intent-A.
...
```
If live remnant nets exist, the instance is FLATTENED (not optimized away) — say so, and flag it as requiring FM verification / re-synthesis rather than silently excluding it. **The Step-1 validator independently detects this: for every `comb_net_force` changed module it re-greps the netlist and emits a non-blocking WARN (`flattened_instance_warnings[]`) for any instance that is neither a clean copy nor truly absent but has a live flattened remnant — so a wrong "optimized away" claim is surfaced rather than shipped silently. It does NOT block: the flow proceeds and ECOs the clean uniquified copies, and FM (Step 6) is the authoritative equivalence gate on the unpatched flattened region — only if FM fails there is re-synthesis warranted. This detection is a HEURISTIC and INCOMPLETE: it only catches remnants that kept the instance-name prefix; an instance whose melted nets were renamed to their output-signal name is invisible to it, so a zero-warning run does NOT prove full coverage — FM remains the only name-agnostic completeness check.**
