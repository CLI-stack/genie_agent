...
- `forced_signals: [{signal, const, assignment_evidence, bits: [{bit, old_net, dff_cell, dff_pin}]}]` — one entry per driven signal (do NOT drop `recdsp_c0vld`). `const` is a Verilog literal. **`assignment_evidence`** is the verbatim RTL RHS text you read (e.g. `"1'b1"`, `"UMC_MOP_CAS"`) — it is your CLASSIFICATION PROOF and is validated to be constant-like. Per bit: `old_net` is that bit's PRE-ECO D-input net, `dff_cell`/`dff_pin` identify the flop to rewire (pin defaults to `D`).
...
2c. **An equality/enum match term added by an `and_term`/`wire_swap` is NEW logic — BUILD it, never PEND it.** When the OR-in/AND-in term is itself a comparison like `(sig == CONST)` / `(opcode == \`ENUM\`)` (e.g. Intent-A `(recdsp_c0mop == UMC_MOP_MRR)`), that match net does NOT exist in PreEco — it must be DECODED into comparator gates (per §2/§2b: AND of per-bit matches, or XOR/XNOR fold) inside `and_term_gate_chain_design`, driving a fresh `n_eco_<jira>_*` net that becomes the combine gate's second input. **NEVER** emit the match as `PENDING_FM_RESOLUTION:<new_token>` and **NEVER** list it in `condition_inputs_to_query` — a PENDING placeholder tells Step 2 to *find* an existing net, but there is none, so FM echo-falls-back and Step 3 substitutes an unrelated ECO net (Intent-A silently wired to the WCK-sync condition in 9666 run 20260704085942). `PENDING_FM_RESOLUTION` is only for a *synthesis-internal signal that already exists* in the gate netlist under a mangled name — not for a term you are introducing. Step 1 validator `eq_decode_issues` hard-fails an `== <CONST>` term that pends its own new_token.
