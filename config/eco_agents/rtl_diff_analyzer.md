...
**`priority_force` — a new branch that FORCES signals to constants (do NOT force-fit into and_term/enable_swap).** When the ECO adds a branch like `else if (<cond>) begin sigA <= CONST_A; sigB <= CONST_B; end` (e.g. LPDDR5 WCK-not-in-sync → `recdsp_c0vld <= 1'b1; recdsp_c0mop <= UMC_MOP_CAS;`), classify as `change_type: priority_force` with:
...
- `forced_signals: [{signal, const, bits: [{bit, old_net, dff_cell, dff_pin}]}]` — one entry per driven signal (do NOT drop `recdsp_c0vld`). `const` is a Verilog literal (`1'b1`, `5'b01011`). Per bit: `old_net` is that bit's PRE-ECO D-input net, `dff_cell`/`dff_pin` identify the flop to rewire (pin defaults to `D`).

Step 3 does NOT hand-build this — the deterministic helper **`eco_emit_priority_force.py`** consumes the schema above and, per bit, emits the const via GATE CHOICE (const-1 bit → `OR2(cond, old_bit)`; const-0 bit → `INR2(A1=old_bit, B1=cond)` = `old & ~cond`) plus one shared `INV(cond)→cond_n`, then a DFF-pin rewire repointing that bit's flop `.D` from `old_net` to the fresh `n_eco_<jira>_pf_*` net (never a driver-rename, never a constant left dangling). Validator: Step 1 requires `condition_gate_chain` + each `forced_signals[].const` a valid constant + non-empty `bits[]` where each bit has `old_net`+`dff_cell`; Step 3 requires, per forced signal, force-mux gate(s) AND a `.D` rewire onto EVERY force-mux output.
