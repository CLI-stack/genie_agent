...
**MULTI-INSTANCE comb_net_force (MANDATORY — same Gap-2 rule as `and_term`/`new_port`).** A single `scope` only exists when the declaring module is instantiated ONCE. A `comb_net_force` edits the **module** (`module_name`), so it affects **EVERY** instantiation of that module — and those instantiations may live in **different parent modules and different RTL source files**, reachable through different hierarchy branches. When `module_name` is instantiated **more than once**, there is NO single scope — leave `scope: null` and instead populate BOTH of:
  1. `instances: ["<full_scope_A>", "<full_scope_B>", ...]` — the FULL tile-relative hierarchy of **every** instance (NOT bare instance names, NOT module names; `instances[i][0]` must never equal `<TILE>`). Step 2's deriver (`eco_fenets_derive_queries.py` Cat 4c) iterates these to qualify the cone leaves per branch.
  2. `nets_to_query` entries — one fully-qualified `{net_path, hierarchy, instance, reason}` per cone leaf **per instance branch** (one full path per `(instance × cone-leaf)`, e.g. `<hier_A>/<leaf>` AND `<hier_B>/<leaf>`). Step 2 consumes these directly (Cat 0 passthrough) — this is the authoritative query source for the multi-instance case, because the deriver's per-change cone walk cannot self-scope a `scope: null` change.

**MANDATORY discovery — enumerate ALL instantiation sites across ALL files (this is where partial coverage silently creeps in).** Do NOT stop at the instances you find in the first RTL file where you noticed the change. A changed module is frequently instantiated by several different parents in several different files. Grep the **entire** SynRtl source set:
```bash
grep -rnE "^[[:space:]]*<module_name>[[:space:]]+[A-Za-z0-9_]+[[:space:]]*\(" <REF_DIR>/data/PreEco/SynRtl/
```
Enumerate **every** hit across **every** file, then trace each instantiating parent up to its full tile-relative hierarchy path. `instances[]` and `nets_to_query` must cover the complete set — an (N−k)/N partial (covering only the instances in one file/parent) leaves the uncovered copies with no cone queries → no ECO → FM non-equivalence on those instances. **The Step-1 validator independently re-greps all SynRtl files for `<module_name> <inst> (` and HARD-FAILS if any instantiation site is not covered by `instances[]`/`nets_to_query`, listing the missing instances and their files — so a partial enumeration cannot slip through.**

Emitting `scope: null` with NEITHER `instances[]` NOR matching `nets_to_query` entries makes Step 2 derive ZERO queries (the deriver has nothing to scope), which historically forced the banned hand-picked fallback. The Step-1 validator hard-fails this (see §comb_net_force multi-instance check).
