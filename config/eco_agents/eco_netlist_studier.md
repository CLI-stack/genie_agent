...
1. `named_net` — choose based on context:
   ...
   - **Exception — CSR/register output bus slots (e.g. `REGCMD.REG_UmcCfgEco[4]`, `REG.oQ_UmcCfgEco_UmcCfgEco[4]`):** use the **flat form of the port name + bit**: `<port_name>_<N>_` (e.g. `REG_UmcCfgEco_4_`). This preserves FM's compare-point traceability — `REG_UmcCfgEco_4_` is the sanitized form of the register output compare point itself, so FM can trace through it without hitting an unmatched boundary. Using a generic `eco_{jira}_` prefix instead creates an extra indirection level that FM cannot resolve across Synth→PrePlace stages (root cause of 10036 FM FAIL: AI used `eco_10036_UmcCfgEco_4` instead of `REG_UmcCfgEco_4_`).
