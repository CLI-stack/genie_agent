...
> **STOP — register guard-change (`target_register` + `branch_assigns`) → DO NOT build the combine here.** If the `and_term` change carries both `target_register` and `branch_assigns` (a guard broaden/tighten on a register branch that assigns a constant), do **NOT** hand-build the OR2/NOR2/INR2 combine gate OR the `.D` rewire. Leave it ENTIRELY to the deterministic `eco_cone_rebuild.py --emit-into-study` pass (function `emit_reg_guard_delta_batch`), exactly like a `comb_net_force` — it rebuilds the flop's D-cone changed region from RTL and re-drives `.D` correct-by-construction (OR vs AND-NOT baked into the region fold by the branch's assigned value). Hand-building `OR2(old_token, new_term)` here is WRONG for a CLEAR branch (`reg <= 1'b0`): it forces the register toward 1 (the JIRA-9666 `postcas` bug). The rule below applies ONLY to combinational term-folds (no `branch_assigns`).
