...
|---|---|---|
| `<AI_ECO_FLOW_DIR>/<TAG>_eco_preeco_study.json` | end of Phase 1 | JSON (skeleton — verifier enriches) |
| `<AI_ECO_FLOW_DIR>/<TAG>_eco_step3_collect.rpt`  | last action before exit | RPT (summary) |
...
- `<AI_ECO_FLOW_DIR>/<TAG>_eco_rtl_diff.json` — Step 1 RTL diff classification
- `<AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_rename_map.json` — Step 2 FM-resolved per-stage map (SOURCE OF TRUTH for Rule 32)
...
> **STOP — register guard-change (`target_register` + `branch_assigns` OR `branch_loads`) → DO NOT build the combine here.** If the `and_term` change carries `target_register` AND either `branch_assigns` (the widened branch assigns a **constant**, e.g. `reg <= 1'b0`) OR `branch_loads` (the widened branch **loads a data expression** on a **clock-gated** register/counter, e.g. `WckSyncCtr0 <= rdwcksyncclks`), do **NOT** hand-build the OR2/NOR2/INR2 combine gate, the `.D` rewire, or any `E`-pin/enable rewire. Leave it ENTIRELY to the deterministic `eco_cone_rebuild.py --emit-into-study` pass (function `emit_reg_guard_delta_batch`), exactly like a `comb_net_force`. It handles BOTH classes correct-by-construction:
> - **constant (`branch_assigns`)** — rebuilds the flop's D-cone changed region from RTL and re-drives `.D` (OR for set / AND-NOT for clear, baked into the region fold by the assigned value). Hand-building `OR2(old_token, new_term)` here is WRONG for a CLEAR branch — it forces the register toward 1 (the JIRA-9666 `postcas` bug).
> - **data-load / clock-gated (`branch_loads`)** — emits a slim per-bit `.D` hold-mux (`D = load_active ? load_val : orig-.D`, reusing the existing physical `.D` net as the else-leg) AND widens the clock-gate enable (`E = old_E | load_active`). Hand-building a single-net OR into the `E`-pin only is WRONG — the flop enables on the new region but loads the WRONG value (the JIRA-9666 `WckSyncCtr0` bug: loaded the decrement path instead of `rdwcksyncclks`).
>
...
**CRITICAL — this rule applies to EVERY non-output pin of EVERY chain gate, not just the primary data input (A1).** Scalar inputs to chain gates — including reset signals (B1), MUX selects (S), and any other non-ECO net — are equally subject to CTS renaming across stages.
...
2. **`<AI_ECO_FLOW_DIR>/<TAG>_eco_fenets_rename_map.json`** — use when Priorities 0 and 1
...
python3 script/eco_scripts/eco_resolve_bus_width.py \
    ...
    --output        <AI_ECO_FLOW_DIR>/<TAG>_eco_bus_width_<target>.json
```
...
python3 script/eco_scripts/eco_emit_dff_entry.py \
    ...
    --output <AI_ECO_FLOW_DIR>/<TAG>_eco_dff_entry_<target>.json
```
...
```python
out = json.load(open(f'{AI_ECO_FLOW_DIR}/{TAG}_eco_dff_entry_{target}.json'))
...
python3 script/eco_scripts/eco_resolve_bus_width.py \
    ...
    --rtl-dir <REF_DIR>/data/SynRtl \
    --preeco-synth <REF_DIR>/data/PreEco/Synthesize.v.gz \
    --output <AI_ECO_FLOW_DIR>/<TAG>_eco_bus_width_<output_net>.json
```
...
2. If 0 (net absent — synthesis merged it into cone): find `<signal_name>_d1_reg` D-input wire → that is `D_INPUT`. **Do NOT use `D_INPUT` directly.** Find the cell driving `D_INPUT` and check its output pin type:
   ...
   - **Driver output is `.Z` (non-inverting: AN/OR/BUF/AN2 prefix)** → `D_INPUT` has extra AND/OR gating mixed in — do NOT use it. Trace to the driver cell's INPUTS and pick the one whose name matches the signal's purpose (e.g. the ND4/NR4 output, not the enable/clock gating input). That traced net (e.g. `ctmn_<n>`) is the **combinational** source. Record `driver_net: <pure_input_of_AND_cell>`.
   ...
   - **🚫 NEVER source a signal from its own `_d<N>` register (CYCLE-CORRECTNESS — Check `SELF-DELAYED-FEED`).** By naming convention `<signal_name>_d<N>` is `<signal_name>` delayed N clocks — a different, stale value. The `_d<N>` flop is a **trace anchor only**: use its `.D` cone to FIND the combinational source, then discard it. Never wire `<signal_name>_d<N>` as the buffer-chain source that reconstructs `<signal_name>` itself — the rebuilt signal would be off by N clocks → FM mismatch. (Substituting a *different* net whose **base name differs** may be legitimate even if that net is registered; the rule keys on a MATCHING base, so only same-base-delayed feeds are the bug.)
   - **The combinational source (`driver_net`) is usually a synthesis-internal net that P&R renames — resolve it PER STAGE.** Synthesis-internal names (`ctmn_*`/`N<n>`/`phfnn_*`, etc.) commonly have 0 refs in PrePlace/Route. For each stage where the Synth `driver_net` is absent, resolve its **combinational** P&R-renamed equivalent via `eco_resolve_synth_internal.py --synth-net <driver_net> --stage <stage>` (or the fenets `find_equivalent_nets` chain) and put THAT net in `port_connections_per_stage`. If a stage cannot be resolved, set that stage's input to `UNRESOLVABLE:<driver_net>` with `confirmed:false` (verifier / Round 2 resolves it) — do **NOT** silently fall back to the `_d<N>` flop.
...
python3 script/eco_scripts/eco_emit_shadow_gate.py \
  ...
  --module    <module_name> \
  --output    <AI_ECO_FLOW_DIR>/<TAG>_eco_shadow_gate_rewires.json
```
...
**When `d_input_has_reset_context: true` on a wire_swap entry**: emit (1) a shared `INVD1(reset_signal) → n_eco_<jira>_ireset_inv` new_logic_gate if not already present, and (2) per-bit `AN2D1(n_eco_<jira>_ireset_inv, mux_out_N) → n_eco_<jira>_nxtd_N_` gates as the final element of the chain. Update D-pin rewires to point to the AN2D1 outputs, not the AO22 mux outputs.
...
Write `<AI_ECO_FLOW_DIR>/<TAG>_eco_preeco_study.json`.
...
**Write collect RPT** to `<AI_ECO_FLOW_DIR>/<TAG>_eco_step3_collect.rpt`:
```
ECO NETLIST STUDIER — COLLECT PASS
