...
2. If 0 (net absent — synthesis merged it into cone): find `<signal_name>_d1_reg` D-input wire → that is `D_INPUT`. **Do NOT use `D_INPUT` directly as the buffer source.** Trace one level deeper — find the cell whose `.Z`/`.ZN` drives `D_INPUT`, then use one of its INPUTS as the pure combinational source. The D-input wire typically has extra AND-gate logic mixed in (enable/clock gating) that makes it functionally different from the clean `<signal>` net. Step 2 fenets result for `<signal_name>_d1` (if available) gives the authoritative per-instance source. Record `driver_net: <pure_source>`, `needs_buffer_chain: true`.
   - Emit: `INV(<pure_source>) → n_eco_<jira>_<signal>_inv1`, `INV(n_eco_<jira>_<signal>_inv1) → <signal_name>`. Step 3 validator CHECK PORT-DECL-WRONG-SOURCE flags if `driver_net` equals the DFF D-input wire instead of its upstream pure source.
