...
- The PreEco study JSON from Step 3: `<AI_ECO_FLOW_DIR>/<TAG>_eco_preeco_study.json` — this is the **fully enriched** JSON written by eco_netlist_verifier (not the initial skeleton from eco_netlist_studier). It contains `port_connections_per_stage` for all 3 stages, auto-added port_declaration and consumer rewire entries, and all GAP-15 corrections.
...
- Output: `<AI_ECO_FLOW_DIR>/<TAG>_eco_applied_round<ROUND>.json`

Wait for eco_applier sub-agent to complete.

**CHECKPOINT:** Verify `<AI_ECO_FLOW_DIR>/<TAG>_eco_applied_round<ROUND>.json` exists and contains a `summary` field. Check that backup files `<REF_DIR>/data/PostEco/<Stage>.v.gz.bak_<TAG>_round<ROUND>` exist for each stage that had confirmed cells. Do NOT continue to Step 5 (Pre-FM Quality Checker) if file is missing.
...
cd <BASE_DIR> && python3 script/eco_scripts/eco_rpt_generator.py step4 \
    ...
    --output  <AI_ECO_FLOW_DIR>/<TAG>_eco_step4_eco_applied_round<ROUND>.rpt
```
...
```bash
cp <AI_ECO_FLOW_DIR>/<TAG>_eco_step4_eco_applied_round<ROUND>.rpt <AI_ECO_FLOW_DIR>/
...
ECO_OUT_DIR=<AI_ECO_FLOW_DIR> bash script/eco_scripts/eco_verilog_validator.sh \
    ...
    <AI_ECO_FLOW_DIR>/<TAG>_eco_applied_round1.json
CHECK8_EXIT=$?
```

Read `<AI_ECO_FLOW_DIR>/<TAG>_eco_verilog_validator_round1.json`. If any stage is FAIL → **do NOT spawn eco_pre_fm_checker yet**. Fix the syntax issues first using the inline fix procedures in eco_pre_fm_checker.md, then re-run eco_verilog_validator.sh. Only proceed when all 3 stages are PASS.

Pass `CHECK8_RESULT_PATH=<AI_ECO_FLOW_DIR>/<TAG>_eco_verilog_validator_round1.json` to the eco_pre_fm_checker sub-agent — it reads this pre-computed result directly (does NOT re-run eco_verilog_validator.sh).
...
- Path to applied JSON: `<AI_ECO_FLOW_DIR>/<TAG>_eco_applied_round1.json`
- `CHECK8_RESULT_PATH=<AI_ECO_FLOW_DIR>/<TAG>_eco_verilog_validator_round1.json` (pre-computed by ORCHESTRATOR — do NOT re-run)
...
```bash
ls <AI_ECO_FLOW_DIR>/<TAG>_eco_pre_fm_check_round1.json || { echo "FAIL: Step 5 pre-FM did not pass (no round json) — do NOT submit FM. Inspect the newest <AI_ECO_FLOW_DIR>/<TAG>_eco_pre_fm_check_round1_iter*.json and re-spawn eco_pre_fm_checker."; exit 1; }
```
...
python3 script/eco_scripts/eco_validate_pre_fm_integrity.py \
    --check-json <AI_ECO_FLOW_DIR>/<TAG>_eco_pre_fm_check_round1.json
...
```python
check = load(f"{AI_ECO_FLOW_DIR}/{TAG}_eco_pre_fm_check_round1.json")
...
if check["passed"]:
    ...
    while not check["passed"] and heal_attempt < MAX_HEAL:
        ...
        spawn eco_pre_fm_checker sub-agent (ROUND=1, CHECK8_RESULT_PATH={AI_ECO_FLOW_DIR}/{TAG}_eco_verilog_validator_round1.json)
        # canonical round json exists ONLY on pass (removed on fail) — absence == not passed
        cj = f"{AI_ECO_FLOW_DIR}/{TAG}_eco_pre_fm_check_round1.json"
...
```bash
ls <AI_ECO_FLOW_DIR>/<TAG>_eco_pre_fm_check_round1.json
```
...
```
Also read `<AI_ECO_FLOW_DIR>/<TAG>_eco_fm_tag_round1.tmp` to get `eco_fm_tag` — save it to `eco_fixer_state` if FM failed.
...
**[eco_fm_runner sub-agent does this — not the ORCHESTRATOR]** eco_fm_runner reads the tag from CLI output, saves it to `<AI_ECO_FLOW_DIR>/<TAG>_eco_fm_tag_round<ROUND>.tmp`, polls `<AI_ECO_FLOW_DIR>/<eco_fm_tag>_spec` every 5 minutes until `OVERALL ECO FM RESULT:` appears.

**[eco_fm_runner sub-agent does this — not the ORCHESTRATOR]** eco_fm_runner parses results and writes `<AI_ECO_FLOW_DIR>/<TAG>_eco_fm_verify.json`. Since all 3 FM targets are always run every round, the JSON is fully updated from the current round — no "carry forward" from prior rounds. Every target has a fresh result:
```json
{
...
**[eco_fm_runner sub-agent does this — not the ORCHESTRATOR]** eco_fm_runner writes `<AI_ECO_FLOW_DIR>/<TAG>_eco_step6_fm_verify_round1.rpt` and copies it to `AI_ECO_FLOW_DIR/`.

**CHECKPOINT:** Verify both `<AI_ECO_FLOW_DIR>/<TAG>_eco_fm_verify.json` and `<AI_ECO_FLOW_DIR>/<TAG>_eco_step6_fm_verify_round<ROUND>.rpt` exist and are non-empty.
...
'''
    ...
    summary = read_json(f'{AI_ECO_FLOW_DIR}/{TAG}_abort_recovery_attempt{attempt}.json')
...
# After loop:
final = read_json(f'{AI_ECO_FLOW_DIR}/{TAG}_eco_fm_verify.json').get('verdict', 'UNKNOWN')
...
def grow_auto_yaml_from_attempts(TAG, BASE_DIR, attempt_count):
    ...
    for n in range(1, attempt_count + 1):
        summary_path = f'{AI_ECO_FLOW_DIR}/{TAG}_abort_recovery_attempt{n}.json'
...
Write `<AI_ECO_FLOW_DIR>/<TAG>_round_handoff.json` **before any signal/spawn**:
...
Then write `<AI_ECO_FLOW_DIR>/<TAG>_eco_fixer_state`:
```json
{
  "round": 1,
  "tag": "<TAG>",
...
```bash
date -Iseconds | xargs -I{} echo "exited {}" > <AI_ECO_FLOW_DIR>/<TAG>_apply_phase_exited.marker
ls -la <AI_ECO_FLOW_DIR>/<TAG>_apply_phase_exited.marker
```
...
| File | Content |
|------|---------|
| `<AI_ECO_FLOW_DIR>/<TAG>_eco_analyze` | Metadata: tile, ref_dir, tag, jira |
| `<AI_ECO_FLOW_DIR>/<TAG>_eco_rtl_diff.json` | RTL diff analysis + nets to query |
| `<AI_ECO_FLOW_DIR>/<fenets_tag>_find_equivalent_nets_raw.rpt` | Raw FM output — all 3 targets concatenated |
| `<AI_ECO_FLOW_DIR>/<TAG>_eco_step2_fenets.rpt` | Step 2 RPT — find_equivalent_nets results |
| `<AI_ECO_FLOW_DIR>/<TAG>_eco_preeco_study.json` | PreEco netlist confirmation |
| `<AI_ECO_FLOW_DIR>/<TAG>_eco_applied_round1.json` | ECO changes applied/inserted/skipped (Round 1) |
| `<REF_DIR>/data/eco_fm_config` | FM run config (fixed filename) |
| `<AI_ECO_FLOW_DIR>/<TAG>_eco_fm_verify.json` | PostEco FM verification results (Round 1) |
| `<AI_ECO_FLOW_DIR>/<TAG>_eco_fixer_state` | Round tracking (if FM fails) |
| `<AI_ECO_FLOW_DIR>/<TAG>_eco_step1_rtl_diff.rpt` | Step 1 RPT (written by rtl_diff_analyzer) |
| `<AI_ECO_FLOW_DIR>/<TAG>_eco_step3_netlist_study_round1.rpt` | Step 3 RPT (written by eco_netlist_studier) |
| `<AI_ECO_FLOW_DIR>/<TAG>_eco_step4_eco_applied_round1.rpt` | Step 4 RPT Round 1 (written by eco_applier) |
| `<AI_ECO_FLOW_DIR>/<TAG>_eco_step6_fm_verify_round1.rpt` | Step 6 RPT Round 1 |
| `<AI_ECO_FLOW_DIR>/<TAG>_round_handoff.json` | Handoff with `next_phase` (ROUND→signal main session; FINAL→spawned inline; STOP→no spawn) |
| `<AI_ECO_FLOW_DIR>/<TAG>_apply_phase_exited.marker` | EXIT sentinel — main session polls for this to confirm clean exit |
