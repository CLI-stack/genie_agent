...
**MANDATORY SECOND ACTION:** Read **only** your scope-contract section in the parent orchestrator: `config/eco_agents/STUDY_ORCHESTRATOR.md` **§STEP 3 — Study PreEco Gate-Level Netlist** (you run as the verify+enrich pass after `eco_netlist_studier`, before `eco_expand_chains.py` + `eco_validate_step3.py`). For per-round re-runs use `config/eco_agents/ROUND_ORCHESTRATOR.md` **§STEP 6f Pass 6f-B** instead. Do NOT read other STEP sections.
...
**Check 3 shadow-gate CP correction:** After resolving all DFF CP pins, scan for new_logic_dff entries in a module that also contains a `clk_gate_ECO_*` shadow gate entry. For each such DFF whose CP is NOT the shadow gate's Q output net (i.e., CP is bare `uclkg` OR any existing non-ECO clock gate): set `port_connections_per_stage[stage]['CP'] = <shadow_gate_Q_net>` for all 3 stages. **Always use the ECO shadow gate Q — never a pre-existing neighbor gate.** Pre-existing gates (e.g. `umcdat_WDB_uclkg_clk_gate_wdbptr_org0_d1_reg`) have different enable conditions and will cause FM cone mismatch.
