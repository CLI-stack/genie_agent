...
Before generating any summary RPT, HTML, or email, copy every JSON / RPT / TXT artifact for this run from `<AI_ECO_FLOW_DIR>/` to `<AI_ECO_FLOW_DIR>/`. The flow dir is the engineer-facing handoff — anything left only in `data/` is invisible to whoever inherits the run.

```bash
# Sync all artifacts: JSON (machine), RPT (human), TXT (logs), HTML (per-round reports)
for f in <AI_ECO_FLOW_DIR>/<TAG>_*.json \
         <AI_ECO_FLOW_DIR>/<TAG>_*.rpt \
         <AI_ECO_FLOW_DIR>/<TAG>_*.txt \
         <AI_ECO_FLOW_DIR>/<TAG>_*.html ; do
...
Read all `<AI_ECO_FLOW_DIR>/<TAG>_eco_applied_round<ROUND>.json` files (ROUND = 1 to TOTAL_ROUNDS) and combine statistics.
...
for round_n in range(1, TOTAL_ROUNDS + 1):
    data = json.load(open(f"{AI_ECO_FLOW_DIR}/{TAG}_eco_applied_round{round_n}.json"))
...
Write `<AI_ECO_FLOW_DIR>/<TAG>_eco_summary.rpt`:
...
LEVELS OF LOGIC (LOL) IMPACT  (deterministic — from eco_lol_impact.py, Synthesize PreEco)
  ...
  SOURCE (canonical): <AI_ECO_FLOW_DIR>/<TAG>_eco_lol_impact.json  — DO NOT hand-estimate.
  If the JSON is missing, print "LOL Impact — not available" (STUDY/ROUND should have written it).
--------------------------------------------------------------------------------

  Read <AI_ECO_FLOW_DIR>/<TAG>_eco_lol_impact.json and render its `summary` then a row per `endpoints[]`:
...
After writing `<AI_ECO_FLOW_DIR>/<TAG>_eco_summary.rpt`, copy to AI_ECO_FLOW_DIR:
```bash
cp <AI_ECO_FLOW_DIR>/<TAG>_eco_summary.rpt <AI_ECO_FLOW_DIR>/
```

**CHECKPOINT:** Verify `<AI_ECO_FLOW_DIR>/<TAG>_eco_summary.rpt` written and non-empty before proceeding.

---

## STEP 7b — Write Final HTML Report

Write `<AI_ECO_FLOW_DIR>/<TAG>_eco_report.html` **using the deterministic script — do NOT write HTML manually**:
...
# → writes <AI_ECO_FLOW_DIR>/<TAG>_eco_report.html with correct CSS/structure
...
| Levels of Logic (LOL) | <AI_ECO_FLOW_DIR>/<TAG>_eco_lol_impact.json (deterministic — eco_lol_impact.py; summary + endpoints[]) |
...
**CHECKPOINT:** Verify `<AI_ECO_FLOW_DIR>/<TAG>_eco_report.html` written and non-empty before proceeding to Step 8.
...
rm -f <AI_ECO_FLOW_DIR>/<TAG>_round_handoff.json
```
...
| File | Content |
|------|---------|
| `<AI_ECO_FLOW_DIR>/<TAG>_eco_summary.rpt` | Summary RPT — statistics + per-step report index |
| `<AI_ECO_FLOW_DIR>/<TAG>_eco_report.html` | Final HTML report (all rounds) |
