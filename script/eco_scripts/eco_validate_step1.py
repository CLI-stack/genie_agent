...
def main():
    ...
    reg_guard_issues = []   # Intent-A register guard-change branch_assigns enforcement (hard-fail)
    for idx, c in enumerate(rtl_diff.get('changes', [])):
        ct = c.get('change_type')
        ...
        if ct == 'priority_force':
            ...
            # old_guard_net and suppress_driver_side are forward-looking fields
            # for a future input-pin-rewire builder. The current driver-side builder
            # does not require them — no validator check here until the builder uses them.
    ...
    if pending_term_issues:
        overall_pass = False
    if reg_guard_issues:            # Intent-A branch_assigns enforcement — HARD-FAIL
        overall_pass = False
    ...
    out = {
        ...
        'reg_guard_issue_count':      len(reg_guard_issues),
        'reg_guard_issues':           reg_guard_issues,
